...
#new
# Function to download audio from YouTube using pytube and convert it to MP3
def download_audio(video_url, output_path):
    try:
        st.status("Downloading audio...")
        yt = YouTube(video_url)
        ...
        video_path = os.path.join(output_path, "audio.mp4")
        video_clip = VideoFileClip(video_path)
        audio_clip = video_clip.audio
        audio_clip.write_audiofile(audio_path)
        st.success(f"Audio downloaded successfully to {audio_path}")
...
def convert_to_wav(input_file, output_file):
    try:
        st.status("Converting to WAV...")
        audio_clip = AudioFileClip(input_file)
        audio_clip.write_audiofile(output_file)
        st.success(f"Converted {input_file} to {output_file}")
    except Exception as e:
        st.error(f"An error occurred while converting to WAV: {e}")
